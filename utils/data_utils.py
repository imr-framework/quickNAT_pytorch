...
def get_imdb_dataset_ksr(data_params):
    ...
    return ImdbData(train, train_label, np.ones((28, 1)), transforms=transform_train), ImdbData(test, test_label,
                                                                                                np.ones((28, 1)))
...
def load_file_paths(data_dir, label_dir, volumes_txt_file=None):
    ...
    file_paths = [
        [os.path.join(data_dir, vol + '.nii'), os.path.join(label_dir, vol + '_BC2_label.nii.gz.nii.gz')]
